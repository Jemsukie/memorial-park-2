import humanize from 'humanize-string'

import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import { QUERY } from 'src/components/Deceased/DeceasedsCell'

const DELETE_DECEASED_MUTATION = gql`
  mutation DeleteDeceasedMutation($id: Int!) {
    deleteDeceased(id: $id) {
      id
    }
  }
`

const MAX_STRING_LENGTH = 150

const formatEnum = (values) => {
  if (values) {
    if (Array.isArray(values)) {
      const humanizedValues = values.map((value) => humanize(value))
      return humanizedValues.join(', ')
    } else {
      return humanize(values)
    }
  }
}

const truncate = (text) => {
  let output = text
  if (text && text.length > MAX_STRING_LENGTH) {
    output = output.substring(0, MAX_STRING_LENGTH) + '...'
  }
  return output
}

const jsonTruncate = (obj) => {
  return truncate(JSON.stringify(obj, null, 2))
}

const timeTag = (datetime) => {
  return (
    datetime && (
      <time dateTime={datetime} title={datetime}>
        {new Date(datetime).toUTCString()}
      </time>
    )
  )
}

const checkboxInputTag = (checked) => {
  return <input type="checkbox" checked={checked} disabled />
}

const DeceasedsList = ({ deceaseds }) => {
  const [deleteDeceased] = useMutation(DELETE_DECEASED_MUTATION, {
    onCompleted: () => {
      toast.success('Deceased deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id) => {
    if (confirm('Are you sure you want to delete deceased ' + id + '?')) {
      deleteDeceased({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>First name</th>
            <th>Last name</th>
            <th>Date born</th>
            <th>Date died</th>
            <th>Latitude</th>
            <th>Longitude</th>
            <th>Status</th>
            <th>Created at</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {deceaseds.map((deceased) => (
            <tr key={deceased.id}>
              <td>{truncate(deceased.id)}</td>
              <td>{truncate(deceased.firstName)}</td>
              <td>{truncate(deceased.lastName)}</td>
              <td>{timeTag(deceased.dateBorn)}</td>
              <td>{timeTag(deceased.dateDied)}</td>
              <td>{truncate(deceased.latitude)}</td>
              <td>{truncate(deceased.longitude)}</td>
              <td>{truncate(deceased.status)}</td>
              <td>{timeTag(deceased.createdAt)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.deceased({ id: deceased.id })}
                    title={'Show deceased ' + deceased.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.editDeceased({ id: deceased.id })}
                    title={'Edit deceased ' + deceased.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete deceased ' + deceased.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(deceased.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default DeceasedsList
